...
pub fn update_card(
    ...
    if new_cart.offers != None {
        let _ = update(cart.filter(id.eq(cart_id)))
            .set(offers.eq(new_cart.offers))
            .execute(conn);
    }
    if new_cart.paid != None {
        let _ = update(cart.filter(id.eq(cart_id)))
            .set(paid.eq(new_cart.paid))
            .execute(conn);
    }
    if new_cart.payment_method != None {
        let _ = update(cart.filter(id.eq(cart_id)))
            .set(payment_method.eq(new_cart.payment_method))
            .execute(conn);
    }

    let single_cart = get_cart(conn, cart_id).unwrap();
    let updated_cart = NewCart::create(
        single_cart.paid,
        single_cart.payment_method.as_deref(),
        single_cart.offers.as_ref(),
    );
